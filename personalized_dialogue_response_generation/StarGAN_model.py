...
class StarGAN:
    def __init__(
            self,
            mode,#pretrain, stargan or test
            size,
            num_layers,
            vocab_size,
            buckets,
            feature_size = 6, # feature size (one-hot)
            baseline = 1.5,
            learning_rate=0.5,
            learning_rate_decay_factor=0.99,
            max_gradient_norm=5.0,
            learning_rate_star=1e-4,
            critic=None,#TODO: dev
            critic_size=None,
            critic_num_layers=None,
            other_option=None,
            use_attn=False,
            output_sample=False,
            input_embed=True,#TODO: dev
            batch_size=32,
        ...
        with variable_scope.variable_scope('BACKWARD') as scope:
            ...
            # backward target weight for pretraining
        ...
        def compute_rf_c(enc_outputs, uniW):
            ...
            for uni, enc_output in zip(uniW, enc_outputs):
                ...
                rf = tf.nn.sigmoid(rf) # pass sigmoid to be in 0 to 1
        ...
        if mode == 'StarGAN':
            ...
            for j, bucket in enumerate(buckets):
                ...
                G_rf_loss = tf.reshape(fake_rf_loss, [-1,])#tf.transpose(fake_rf_loss)
    ...
    def train_StarGAN(
            ...
            sess,
            real_words, 
            forward_decoder_inputs, # only <GO>
            target_feature, 
            backward_decoder_inputs, # <GO> + real_words
            backward_weights,
            original_feature,
            real_data,
            real_feature,
        ...
        print(real_words)
        print("------------------------------")
        print(forward_decoder_inputs)
        print("------------------------------")
        print(target_feature)
        print("------------------------------")
        print(backward_decoder_inputs)
        print("------------------------------")
        ...
        for l in range(decoder_size):
            ...
            input_feed[self.forward_decoder_inputs[l].name] = forward_decoder_inputs[l] # only <GO> is enough
