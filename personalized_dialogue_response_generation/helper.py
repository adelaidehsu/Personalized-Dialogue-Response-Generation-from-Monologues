...
def create_feature(feature_path, feature_size, feature, tokenizer=None, normalize_digits=True):
    #print("Creating feature %s from data %s" % (feature_path))
    voc_feature = []
    voc_feature.append({})
...
def prepare_data(feature_path, feature_size, data_dir, data_path, train_path, vocabulary_size, tokenizer=None):
    ...
    data_to_token_ids(data_path, data_ids_path, vocab_path, tokenizer, normalize_digits=False)
    print ('addy\' vocab is built.')
    return (feature, data_ids_path, train_ids_path)



def tmp_initialize_feature(feature_path, tokenizer=None, normalize_digits=True):
    voc_feature = []
    #print("Creating feature %s from data %s" % (feature_path))
    for x in range(6):
        tmp_feature = {}
        voc_feature.append(tmp_feature)
    with gfile.GFile(feature_path, mode="rb") as f:
        counter = 0
        for line in f:
            counter += 1
            ...
            for id, w in enumerate(tokens):
                ...
                if word in voc_feature[id]:
                    ...
                    voc_feature[id][word] = len(voc_feature[id])+1
